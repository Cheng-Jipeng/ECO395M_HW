...
the sum of **# of major delayed flights** and **# of cancelled flights** divided by the **# from AUS to Destination**,

where # refers to the number of flights. 

The *yearly top 10 worst destination airports* of 2008 together with their "Bad Index" are shown in Figure 1. **Newark airport** is the worst airport given our rating system: the flights from Austin to Newark are most likely to be delayed by over 30 minutes or cancelled. The second follows **JFK airport** in New York. Considering that EWR also serves The NYC Metropolitan Area, it seems that choosing **New York** as destination is the real cause. Busy city and busy airports!
```{r fig1, fig.align='center', fig.cap= "Figure 1: Top 10 Worst Destination of 2008", message = FALSE, echo = FALSE}
...
```
We can also visualize the top 10 airports on a U.S. map in Figure 2 with blue bar refering to the values of their "Bad Index".
```{r fig2, fig.align='center', fig.cap= "Figure 2: Top 10 Worst Destination of 2008 on Map"}
...
ggplot(bad_airports_top10) + usamap +
  ...
  theme_void()
```
If you are also interested in if there are many minor delays in these bad flights, Figure 3 shows the proportions of all kinds of flights' states. If *minor delays can annoy you very much too*, then you must avoid choosing **Atlanta airport** as your destination among the top 10 worst airports.

```{r fig3, fig.align='center', fig.cap= "Figure 3: Distribution of Delays in Top 10 Worst Destination of 2008", echo = FALSE}
...
Does the top list change over the months in 2008? Table 1 answers the question in details by focusing on the top 5 worst airports in each month. Definitely there are new blood across different months like Boston airport in February, Ontario airport in April.  
...
knitr::kable(by_month_table, caption = "Table 1: Top 5 Worst Destinations across Months")
```

A more sketchy way to capture the variability is counting how many time airports occur in the monthly lists of top 5 worst. This is given by Figure 4. There are 23 airports in all of the monthly lists, and this also support our conclusion from yearly list: all 12 lists contains EWR and 6 lists contains JFK.

```{r fig4, fig.align='center', fig.cap= "Figure 4: Frequency of Appearances in Top 5 Worst Destinations across Months", echo = FALSE}
...
```{r fig5, fig.align='center', fig.cap="Figure 5: Delay Rate by Month in 2008"}
abia_modified = abia %>%
...
knitr::kable(top_song_table, caption = "Table 2: Top 10 Most Popular Songs Since 1958",
               col.names = c("Performer","Song","Count"))
```
## Part B
```{r fig6, fig.align='center', fig.cap= "Figure 6: Musical Diversity Over Time", echo = FALSE}
# Part B
billboard %>%
...
```
## Part C
```{r fig7, fig.align='center', fig.cap="Figure 7: Artists Having over 30 Ten-Week-Hits Songs", echo = FALSE}
# Part C
...
## Part C
```{r fig8, fig.align='center', fig.cap="Figure 8: Increasing Trend of Age Similar Across Female and Male Swimmers' Age After Female Participation in Olympics", echo = FALSE}
# Part C
olympics_top20 %>%
...
# K-nearest neighbors
```{r fig9, fig.align='center', fig.cap="Figure 9: RMSE vs. K", echo = FALSE, warning  = FALSE}
...
```{r fig10, fig.align='center', fig.cap="Figure 10: Prediction vs. Testing Set", echo = FALSE}
