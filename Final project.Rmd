...
## Model
\flushleft
In order to analyze which feature will help app users attract more people, we decide to use **Lasso regression** and **Random Forest** methods to study it. As we all know, Lasso regression as a linear regression will give us a better fitted coefficient value. Random forest method, on the other hand, will give us a better fit but it can't give us a good fitted coefficient value. In order to solve the above problems including in each method, we decide to combine these two methods in order to have a deeper study on which characteristics will help app user attract more people.
...
\flushleft
Then we study the partial importance of each variable which we observe in our variable importance plots. 
```{r echo=FALSE, message=FALSE, warning=FALSE}
...
# Conclusion
As female user, in order to attract more people, they need to provide more details in their profile and also post more pictures. These behaviors will provide more information about themselves. In addition, mature female seems to attract more people and new users seem to be more attractive. 
