...
answer: In this question of "Crime" and "Police", there is a problem of endogeneity circle. if we use the simple linear regression model $$y=beta_1+beta_2 * x+u$$, there is a problem of endogeneity, i.e.
E(xu)≠0. Under is circumstance, inconsistency of OLS arises due to the fact that
changes in x(police force) are associated not only with changes in y(crime), but
also change in u.
...
The results suggest that random forests mode have the best performance on the testing data.
...
```
By using the tree model to build our predictive model, we identify the association between rental revenue and green certification. We first split the new greenbuildings data set into training and test set, then we use two different tree methods: random forest and boosted regression trees to build our model. Then we calculate the out of sample RMSE. The result shows that the RMSE of the random forest model is lower that that using boosted regression trees. Therefore, we choose to use random forest method.
...
**Figure 1:** Figure of all features descending by importance

From Figure 1, we found that the most important features that landlords should consider are "size, stories, age", these variables seems to be the most important. However, the "green_rating" variable is not that important from our observation.
...
We then calculate the average change in revenue per square foot for buildings with or without green certification. From the table above, we found that buildings with green certification get more revenue than those without green certification, but the impact is small, considering the difference is about 2 unit.
...
From our analysis, we build the model using random forest to estimate the association between green certification and revenue per square foot per year. From our results, we found that green certification has some positive impact on revenue, but the impact isn't very significant. In addition, we found that these variables "size, age, stories" have more impact on revenue. Based on our research, the landlords should weigh the pros and cons of getting a green certification, thinking critically about the cost and revenue change of getting a green certification. If they figure that the income change doesn't overweigh the cost of getting a certification, they should focus on other features, which have more important impact on revenue.
...
cbind(modelr::rmse(lm_medium, cahousing_test), 
  ...
  knitr::kable(caption = "The RMSEs of Models",
               col.names = c("lm_medium", "lm_step", "dengue_boost", "cahousing_forest", "cahousing_boost")) %>%
  kable_styling(full_width = F)
```
